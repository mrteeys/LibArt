...
function Borrowing($isGet)
{
    ...
    if ($bBarcode == null) {
        echo Error("bBarcode can not be null");
        return;
    }

    if ($rBarcode == null) {
        echo Error("rBarcode can not be null");
        return;
    }
    ...
    if ($rs == null) {
        echo Error("book doesn't exist");
        return;
    }

    if ($rs["State"] == 1) {
        echo Error("have already borrowed");
        return;
    }

    if ($rs["Disabled"]) {
        echo Error("book already disabled");
        return;
    }
